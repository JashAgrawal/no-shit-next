...
export default function AgentChatPage() {
  ...
  const messages = useChatStore(
    useShallow((state) => {
      ...
      return state.agentChats[activeIdeaId]?.[agentId] || [];
    })
  );
    ...
    const fetchHistory = async () => {
      ...
      try {
        const res = await fetch(
          `/api/messages?ideaId=${activeIdeaId}&chatType=agent&agentId=${agentId}`
        );
        if (res.ok) {
          const data = await res.json();
          useChatStore
            .getState()
            .syncFromServer(activeIdeaId, "agent", data.messages, agentId);
        }
      } catch {}
    };
    fetchHistory();
        ...
        <div className="space-y-4 max-w-4xl mx-auto">
          {messages.length === 0 && agent && (
            <div className="text-center py-12 space-y-4 max-w-2xl mx-auto">
              ...
              <div className="border border-border p-4 text-left">
                ...
                <div className="flex flex-wrap gap-2">
                  {agent.expertise.map((exp) => (
                    <span
                      key={exp}
                      className="text-xs font-mono bg-muted px-2 py-1"
                    >
                      {exp}
                    </span>
                  ))}
              ...
              <p className="text-sm font-mono text-muted-foreground">
                Direct line to {agent.name}. Ask anything about{" "}
                {agent.expertise[0]}.
              </p>
            </div>
          )}
